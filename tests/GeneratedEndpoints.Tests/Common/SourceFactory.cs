...
public static class SourceFactory
{
    ...
    )
    {
        ...
        {
            var timeoutArgument = string.IsNullOrWhiteSpace(requestTimeoutPolicy) ? "" : $"(\"{requestTimeoutPolicy}\")";
    ...
    )
    {
        ...
        builder.AppendLine();
        builder.AppendLine("    public static void Configure<TBuilder>(TBuilder builder"
                           + (configureWithServiceProvider ? ", IServiceProvider services" : "")
                           + ")"
        );
    ...
    )
    {
        ...
        if (includePut)
            builder.AppendLine(
                "    [MapPut(\"/matrix/{id:int}\")] public static Results<NoContent, NotFound> Put(int id) => id > 0 ? TypedResults.NoContent() : TypedResults.NotFound();"
            );
    ...
    )
    {
        ...
        builder.AppendLine(includeBindingNames ? "        [FromRoute(Name = \"route-id\")] int id," : "        [FromRoute] int id,");
        builder.AppendLine(includeBindingNames ? "        [FromQuery(Name = \"filter-term\")] string? filter," : "        [FromQuery] string? filter,");
        builder.AppendLine(includeBindingNames ? "        [FromHeader(Name = \"x-trace-id\")] string? traceId," : "        [FromHeader] string? traceId,");
